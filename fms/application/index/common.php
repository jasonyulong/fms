...
if (!function_exists('check_logintoken')) {
    /**
     * 校验是否在线
     * @param $keeplogin 登录keep
     * @param $token 登录token
     * @return mixed
     */
    function check_logintoken($keeplogin, $token)
    {
        $auth = \app\index\library\Auth::instance();
        return $auth->checkKeepLogin($keeplogin, $token);
    }
}
...
if (!function_exists('build_heading')) {
    ...
    function build_heading($path = NULL, $container = TRUE)
    {
        ...
        $data = Db::name('admin_rule')->where('name', $path)->field('title,remark')->find();
        if ($data) {
            $title   = __($data['title']);
